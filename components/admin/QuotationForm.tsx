...
export default function QuotationForm({ 
  ...
  useEffect(() => {
    if (quotation) {
      ...
      setVehicleType(quotation.vehicleDetails?.type || '');
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      setMessage(quotation?.id ? 'Failed to update quotation' : 'Failed to create quotation');
    } finally {
      setLoading(false);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-5 rounded-lg border border-blue-200">
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-gray-500 pt-2">
            Valid for {validityDays} days from creation date
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t">
        ...
        <button 
          ...
          disabled={loading}
        >
          {loading ? (quotation?.id ? 'Updating…' : 'Creating…') : (quotation?.id ? 'Update Quotation' : 'Create Quotation')}
        </button>
      </div>
    </form>
  );
