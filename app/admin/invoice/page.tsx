...
export default function InvoicesPage() {
  ...
  function displayName(inv: any) {
    if (!inv) return 'Unknown';
    if (inv.isB2B) return inv.companyName || inv.customerName || 'Unknown';
    return inv.customerName || inv.companyName || 'Unknown';
  }
  ...
  async function handleDelete(id: string, customerName: string) {
    if (!confirm(`Are you sure you want to delete invoice for ${customerName}? This action cannot be undone.`)) return;

    try {
      await deleteDoc(doc(db, 'invoices', id));
      setDeleteStatus({ type: 'success', message: `Invoice for ${customerName} deleted successfully` });
      setTimeout(() => setDeleteStatus(null), 3000);
    } catch (err: any) {
      safeConsoleError('Delete invoice error', err);
      setDeleteStatus({ type: 'error', message: 'Error deleting invoice: ' + err.message });
                ...
                {paginated.map((inv) => (
                  <div key={inv.id} className="p-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`px-2 py-1 rounded text-xs font-semibold whitespace-nowrap ${inv.paymentStatus === 'paid'
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      {inv.paymentStatus === 'partial' && inv.partialPaymentNotes && (
                        <span title={inv.partialPaymentNotes} className="inline-flex items-center text-yellow-700 ml-1">
                    ...
                    {paginated.map(inv => (
                      <tr key={inv.id} className="border-t hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 font-medium">
                          AED {(inv.total || 0).toFixed(2)}
                          {inv.paymentStatus === 'partial' && inv.partialPaymentNotes && (
                            <span title={inv.partialPaymentNotes} className="inline-flex items-center text-yellow-700 ml-1 align-middle">
                              <svg className="w-4 h-4 mr-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V4a2 2 0 10-4 0v1.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                              <span className="text-xs font-medium">Notes</span>
                            </span>
                          )}
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2 py-1 rounded text-xs font-medium inline-block ${inv.paymentStatus === 'paid'
                              ? 'bg-green-100 text-green-800'
                              : 'bg-red-100 text-red-800'
                            }`}>
                            {(inv.paymentStatus || 'unpaid').toUpperCase()}
                  ...
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1 border rounded text-sm hover:bg-gray-50 ${currentPage === pageNum
